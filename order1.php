...
/*
        ...
        if (is_array($foods)) {
            foreach ($foods as $food) {
              ...
              if($food['fldProduct']==htmlentities($_POST[$food['fldProduct']], ENT_QUOTES, "UTF-8")){

              $myarray[]=htmlentities($_POST[$food['fldProduct']], ENT_QUOTES, "UTF-8");
              $temp .= $food['fldProduct'];
...
*/
        ...
        $varfood="";
    //    $varfood=implode(',', $_POST['checkarray']);
        $varprice="";
...
/*
        if (is_array($foods)) {
            foreach ($foods as $food) {
              ...
              if($food['pmkFdId']==$_POST["checkarray"]){


              $varfood .= $food["fldProduct"].",";
              $varprice .= $food['fldPrice'];
...
if (!$dataEntered){
 ...
 ?>
    ...
    <?php if ($emailERROR) print 'class="form-group has-error has-feedback"'; ?>
                       onfocus="this.select()"
                       autofocus>
            </label>
          </div>
...
<?php if ($nameERROR) print 'class="form-group has-error has-feedback"'; ?>
                       onfocus="this.select()"
                       autofocus>
            </label>
        </div>
 ...
 <div class="form-group">
      ...
      $closetime = strtotime('20:00:00');

      print '<label for="lstpickuptime"';
      if($pickuptimeERROR){
      print ' class="form-control has-error has-feedback" ';
      }
      print '>Pick up time ';
...
if (is_array($foods)) {
    foreach ($foods as $food) {
      print '<label class="form-check-label" for="'.$food['fldProduct'].'">'.$food['fldProduct'];
        print '<input class="form-check-input" type="checkbox" name="checkarray[]" id="checkarray[]" value="'.$food['pmkFdId'].'">';
        print '</label>';
    }
  }
...
?>
            ...
            <div class="form-group">
                <label for="txtinstruction" >Special Instruction
                    <input class="form-control" type="text" id="txtinstruction" name="txtinstruction"
                           value="<?php print $instruction; ?>"
                            placeholder="Enter your special instruction"
            <?php if ($instructionERROR) print 'class="form-group has-error has-feedback"'; ?>
                           onfocus="this.select()"
                           autofocus>
                </label>
            </div>
